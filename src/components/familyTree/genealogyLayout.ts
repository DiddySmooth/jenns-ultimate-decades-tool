...
const GAP_X = 16;   // gap between unrelated family groups
const GAP_SIBLING = 6; // gap between siblings within same family
const GAP_COUPLE = 14; // gap between spouses
const GAP_Y = 200;  // extra room for heart + child lines below cards
const GAP_UNION_GROUP = 14; // visual gutter between adjacent union groups in shared-parent strips
...
export function genealogyLayout(nodes: Node[], edges: Edge[]): { nodes: Node[]; edges: Edge[] } {
  ...
  for (const [, partnersSet] of unionPartnersAll) {
    ...
    // Skip if partners are on different generation rows (cross-gen union)
    const ys = partnerPositions.map((p) => p.y);
    if (Math.max(...ys) - Math.min(...ys) > 10) continue;
    const sharedY = Math.min(...ys);
  ...
  for (const [anchorId, unionIds] of simToUnionIds) {
    ...
    // Phase 1: compute each union's slot within the full cluster width,
    //          place children in those slots, record info for re-centering.
    type UnionChildInfo = {
      uid: string;
